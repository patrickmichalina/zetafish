...
//returns an int array of the given player's hand. returns null if no player exists. 
  /**
   * returns an int array of the given player's hand. returns null if the player has no 
   * cards and or if no player exists. 
...
/**
 * returns an int array of the given player's active hand. returns null if no player exists.
 * Does not return won cards in the players hand.  
 * Returns null if player has no cards. 
 * @param pl player number 
 * @return int []
 */
public int[] getActiveHand(int pl) {
	  int hn[]; 
	  int i = 0;
	  ...
	  //get the player's cards. 
	  	  while(x.hasNext()){
	  		  Card c = x.next(); 
		  	  if((c.getPlayer() == pl)&&(!c.isWon())&&(c.isinPlay())){
			     test[i] = c.getNum();
			     i++;
		  	  }
...
return hn;
	  ...
	  int i = nm; 
	   ...
	   int index =0;
	  	  while(inter < 4){
	  		  ...
	  		  while(d.hasNext()){
	  			  Card c = d.next(); 
	  			  if((c.getNum() == i)&& (c.getPlayer() == p1)&&(!c.isWon())&&(c.isinPlay())){
  ...
  /** 
   *  returns number of new books found in the player's hand, returns 0 if no new books found. 
  ...
  public int getBooks(int pl) {
	  ...
	  //boolean book = false; 
	  int k[] = {1,14,27,40}; 
	  int hand[] = this.getHand(pl); 
	  while(j<13){
		  System.out.println("Checking for book: "+(j+1));
		  j++;
		  int found = 0; 
		  ...
		  if(found==4){
			  ...
			  while(iter.hasNext()){
				  ...
				  if(card.getNum() == k[o]){
					  card.setWon(true); 
					  System.out.println("Card set to won."); 
				  }
			  }
		  }
		//increment k
		  for(int e=0; e<k.length; e++)k[e]++;
	  }//end while
	   
  return i;
  }
  /** 
   *  returns number of  books found in the player's hand, returns 0 if no books found or no player found. 
