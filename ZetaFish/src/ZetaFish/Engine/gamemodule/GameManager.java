...
public GameManager(boolean init, int playernm) {
   ...
   *  ONLY used by initiator. Returns null if no cards found.
   */
  ...
  /** 
   *  Returns an int array of the cards in the specified players hand. 
   *  only returns active cards in the hand. null if no cards.
   *  ONLY used by initiator. 
   */
  public int[] getActiveHand(int p) {
	  int hand[] = this.myGameEngine.getActiveHand(p);
  return hand;
  }
  /** 
  ...
  public int getScore(int p) {
	  ...
	  System.out.println("The score found for " + p + " was " + x);
	  this.updateScore(p, x);
  return x;
  }
